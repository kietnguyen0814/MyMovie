...
let CustomAlertActionEnabledDidChangeNotification = "CustomAlertActionEnabledDidChangeNotification"

protocol DisplayCustomAlert {
    func showCustomAlertWith(title: String, message: String, completion: @escaping (() -> Void))
}

public enum CustomAlertActionStyle : Int {
    case `default`
    case cancel
    case destructive
}

public enum CustomAlertControllerStyle : Int {
    case actionSheet
    case alert
}

// MARK: CustomAlertAction Class

open class CustomAlertAction : NSObject, NSCopying {
    open var title: String
    open var style: CustomAlertActionStyle
    var handler: ((CustomAlertAction?) -> Void)!
    open var enabled: Bool {
        didSet {
            if (oldValue != enabled) {
                NotificationCenter.default.post(name: Notification.Name(rawValue: CustomAlertActionEnabledDidChangeNotification), object: nil)
            }
        }
    }
    
    required public init(title: String, style: CustomAlertActionStyle, handler: ((CustomAlertAction?) -> Void)!) {
        self.title = title
        self.style = style
    ...
    @objc required public init(titleDefault: String, handler: ((CustomAlertAction?) -> Void)!) {
    ...
    @objc required public init(titleCancel: String, handler: ((CustomAlertAction?) -> Void)!) {
        self.title = titleCancel
        self.style = .cancel
        self.handler = handler
        self.enabled = true
    }
    ...
    open func copy(with zone: NSZone?) -> Any {
        switch style {
        case .default:
            let copy = type(of: self).init(titleDefault: title, handler: handler)
            copy.enabled = self.enabled
            return copy
            
        case .cancel:
            let copy = type(of: self).init(titleCancel: title, handler: handler)
            copy.enabled = self.enabled
            return copy
...
// MARK: CustomAlertAnimation Class

open class CustomAlertAnimation : NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func presentAnimateTransition(_ transitionContext: UIViewControllerContextTransitioning) {
        ...
        let alertController = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.to) as! CustomAlertController
    ...
    func dismissAnimateTransition(_ transitionContext: UIViewControllerContextTransitioning) {
        ...
        let alertController = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.from) as! CustomAlertController
...
// MARK: CustomAlertController Class
@objc(CustomAlertController)
open class CustomAlertController : UIViewController, UITextFieldDelegate, UIViewControllerTransitioningDelegate {
    
    // Message
    open var message: String?
    
    // AlertController Style
    fileprivate(set) var preferredStyle: CustomAlertControllerStyle?
    ...
    open var alertViewBgColor = UIColor(red:1, green:1, blue:1, alpha:1.0)
    fileprivate var alertViewWidth: CGFloat = UIScreen.main.bounds.width - 30
    fileprivate var alertViewHeightConstraint: NSLayoutConstraint?
    fileprivate var alertViewPadding: CGFloat = 15.0
    fileprivate var innerContentWidth: CGFloat = UIScreen.main.bounds.width - 30 - 30
    ...
    open var titleFont : UIFont {
        get {
            if (UIDevice.current.userInterfaceIdiom == .pad) {
                return UIFont(name: "System-Bold", size: 22) ?? UIFont.boldSystemFont(ofSize: 22)
            } else {
                return UIFont(name: "System-Bold", size: 20) ?? UIFont.boldSystemFont(ofSize: 20)
            }
        }
    }
    //    open var titleFont : UIFont {
    //    get {
    //        if (UIDevice.current.userInterfaceIdiom == .pad) {
    //            return UIFont(name: "System-Bold", size: 22) ?? UIFont.systemFont(ofSize: 22)
    //        } else {
    //            return UIFont(name: "System-Bold", size: 20) ?? UIFont.systemFont(ofSize: 20)
    //        }
    //    }
    //    open var titleTextColor = UIColor(red:77/255, green:77/255, blue:77/255, alpha:1.0)
    open var titleTextColor = UIColor(red:0/255, green:0/255, blue:0/255, alpha:1.0)
    ...
    open var buttonFont: [CustomAlertActionStyle : UIFont] = [
    ...
    open var buttonTextColor: [CustomAlertActionStyle : UIColor] = [
        .default : UIColor.white,
        .cancel  : UIColor.white,
        .destructive  : UIColor.white
    ]
    open var buttonBgColor: [CustomAlertActionStyle : UIColor] = [
        .default : UIColor(red:52/255, green:152/255, blue:219/255, alpha:1),
        //        .default : UIColor(red:41/255, green:198/255, blue:77/255, alpha:1),
    ...
    @objc public func setDefaultButtonBgColor(color: UIColor) {
        buttonBgColor[.default] = color
    }
    
    @objc public func setCancelButtonBgColor(color: UIColor) {
        buttonBgColor[.cancel] = UIColor(red:127/255, green:140/255, blue:141/255, alpha:1)
    }
    
    @objc public func setDestructiveButtonBgColor(color: UIColor) {
        buttonBgColor[.destructive] = UIColor(red:231/255, green:76/255, blue:60/255, alpha:1)
    }
    
    open var buttonBgColorHighlighted: [CustomAlertActionStyle : UIColor] = [
    ...
    @objc public func setDefaultButtonHightlightBgColor(color: UIColor) {
        buttonBgColorHighlighted[.default] = color
    }
    
    @objc public func setCancelButtonHightlightBgColor(color: UIColor) {
        buttonBgColorHighlighted[.cancel] = UIColor(red:140/255, green:152/255, blue:153/255, alpha:1)
    }
    
    @objc public func setDestructiveButtonHightlightBgColor(color: UIColor) {
        buttonBgColorHighlighted[.destructive] = UIColor(red:234/255, green:97/255, blue:83/255, alpha:1)
    }
    ...
    // Initializer
    public convenience init(title: String?, message: String?, preferredStyle: CustomAlertControllerStyle) {
        ...
        // NotificationCenter
        NotificationCenter.default.addObserver(self, selector: #selector(CustomAlertController.handleAlertActionEnabledDidChangeNotification(_:)), name: NSNotification.Name(rawValue: CustomAlertActionEnabledDidChangeNotification), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(CustomAlertController.handleKeyboardWillShowNotification(_:)), name: UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(CustomAlertController.handleKeyboardWillHideNotification(_:)), name: UIResponder.keyboardWillHideNotification, object: nil)
        //        NotificationCenter.default.addObserver(self, selector: #selector(CustomAlertController.handleKeyboardWillShowNotification(_:)), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        //        NotificationCenter.default.addObserver(self, selector: #selector(CustomAlertController.handleKeyboardWillHideNotification(_:)), name: NSNotification.Name.UIResponder.keyboardWillHideNotification, object: nil)
        //        #selector(CustomAlertController.handleContainerViewTapGesture(_:))
        // Delegate
        self.transitioningDelegate = self
    }
    
    @objc public func initAlert(title: String?, message: String?) {
        self.title = title
        self.message = message
    ...
    override open func viewDidAppear(_ animated: Bool) {
        ...
        if (!isAlert() && cancelButtonTag != 0) {
            let tapGesture = UITapGestureRecognizer(target: self, action: #selector(CustomAlertController.handleContainerViewTapGesture(_:)))
    ...
    open func layoutView(_ presenting: UIViewController?) {
        ...
        if (!isAlert()) {
            ...
            if (UIDevice.current.userInterfaceIdiom == .pad) {
                alertViewWidth = 370
                innerContentWidth = alertViewWidth - 30
            }
        }
        //        print("Alert Width: ",alertViewWidth)
        ...
        if (hasMessage) {
            ...
            //            messageView.frame = CGRect(x: innerContentWidth / 2 - (innerContentWidth * 2 / 3) / 2, y: textAreaPositionY, width: innerContentWidth * 2 / 3, height: messageView.frame.height)
        ...
        if (isAlert() && buttons.count == 2) {
            ...
            for button in buttons {
                let action = actions[button.tag - 1] as! CustomAlertAction
            ...
            for button in buttons {
                let action = actions[button.tag - 1] as! CustomAlertAction
                if (action.style != CustomAlertActionStyle.cancel) {
                    button.titleLabel?.font = buttonFont[action.style]
                    button.setTitleColor(buttonTextColor[action.style], for: UIControl.State())
                    button.setBackgroundImage(createImageFromUIColor(buttonBgColor[action.style]!), for: UIControl.State())
                    button.setBackgroundImage(createImageFromUIColor(buttonBgColorHighlighted[action.style]!), for: .highlighted)
            ...
            if (cancelButtonTag != 0) {
                ...
                let action = actions[cancelButtonTag - 1] as! CustomAlertAction
                button.titleLabel?.font = buttonFont[action.style]
                button.setTitleColor(buttonTextColor[action.style], for: UIControl.State())
                button.setBackgroundImage(createImageFromUIColor(buttonBgColor[action.style]!), for: UIControl.State())
                button.setBackgroundImage(createImageFromUIColor(buttonBgColorHighlighted[action.style]!), for: .highlighted)
    ...
    @objc open func addAction(_ action: CustomAlertAction) {
        // Error
        if (action.style == CustomAlertActionStyle.cancel) {
            for ac in actions as! [CustomAlertAction] {
                if (ac.style == CustomAlertActionStyle.cancel) {
                    let error: NSError? = nil
                    NSException.raise(NSExceptionName(rawValue: "NSInternalInconsistencyException"), format:"CustomAlertController can only have one action with a style of CustomAlertActionStyleCancel", arguments:getVaList([error ?? "nil"]))
                    return
                }
            }
        }
        ...
        button.addTarget(self, action: #selector(CustomAlertController.buttonTapped(_:)), for: .touchUpInside)
    ...
    open func addTextFieldWithConfigurationHandler(_ configurationHandler: ((UITextField?) -> Void)!) {
        ...
        // You can add a text field only if the preferredStyle property is set to CustomAlertControllerStyle.Alert.
        if (!isAlert()) {
            let error: NSError? = nil
            NSException.raise(NSExceptionName(rawValue: "NSInternalInconsistencyException"), format: "Text fields can only be added to an alert controller of style CustomAlertControllerStyleAlert", arguments:getVaList([error ?? "nil"]))
            return
        }
        if (textFields == nil) {
            textFields = []
